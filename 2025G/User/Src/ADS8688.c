...
#define ADS8688_RANGE_CH2_REG     0x07U
#define ADS8688_RANGE_CH3_REG     0x08U
#define ADS8688_AUTO_SEQ_CH0_CH1  0x03U  /* CH0(丝印CH1,坏)+CH1(丝印CH2) */
#define ADS8688_POWER_DOWN_CH3_7  0xF8U  /* CH0/1/2 使能, CH3~7 断电 */
...
ADS8688_Status_t ads8688_set(void)
{
    ...
    ADS8688A_Write_Program_Register(ADS8688_POWER_DOWN_REG, ADS8688_POWER_DOWN_CH3_7);
    Delay(0x100U);
    status = ADS8688_CheckRegister(ADS8688_POWER_DOWN_REG,
                                   ADS8688_POWER_DOWN_CH3_7,
    ...
    Set_CH_Range_Select(ADS8688_RANGE_CH2_REG, ADS8688_RANGE_PM_5V12);
    Delay(0x100U);
    if ((ADS8688A_READ_Program_Register(ADS8688_RANGE_CH2_REG) & 0x0FU)
        != (ADS8688_RANGE_PM_5V12 & 0x0FU)) {
        status = ADS8688_ERR_RANGE_CH2;
        goto done;
    }
...
void ADS8688_Service(void)
{
    ...
    __disable_irq();
    v_suoxiang[0] = sample[0];  /* CH0(丝印CH1,坏) */
    v_suoxiang[1] = sample[1];  /* CH1(丝印CH2,THS4001) */
...
/* 手动读 CH1(丝印CH2,THS4001) */
uint16_t ADS8688_ReadCH1(void)
{
    uint16_t val = 0;
    if (ADS8688_BeginAccess() == 0U) return 0;
    MAN_Ch_n_Mode(MAN_Ch_1);
    Delay(0x2000U);  /* 给足转换时间 */
    Get_MAN_Ch_n_Mode_Data();  /* 丢弃第一次(可能未稳定) */
    Delay(0x2000U);
    val = Get_MAN_Ch_n_Mode_Data();  /* 第二次才是有效值 */
    ADS8688_EndAccess();
    return val;
}

/* 手动读 CH2(丝印CH3,AD637) */
uint16_t ADS8688_ReadCH2(void)
{
    uint16_t val = 0;
    if (ADS8688_BeginAccess() == 0U) return 0;
    MAN_Ch_n_Mode(MAN_Ch_2);
    Delay(0x2000U);
    Get_MAN_Ch_n_Mode_Data();
    Delay(0x2000U);
    val = Get_MAN_Ch_n_Mode_Data();
    ADS8688_EndAccess();
    return val;
}
