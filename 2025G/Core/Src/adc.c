...
/* USER CODE BEGIN 1 */
/**
  * @brief  启动 ADC1 + DMA 采样
  * @param  Size: 本次采样点数
  * @note   数据会存入 ADC1_ConvertedValue[]
  */
HAL_StatusTypeDef ADC_DMA_Trig(uint16_t Size)
{
    if (Size == 0 || Size > ADC1_DMA_Size)
    {
        return HAL_ERROR;
    }
...
/**
  ...
  * @note   DMA 采满 ADC1_ConvertedValue[] 后会进入这里
  */
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    if (hadc->Instance == ADC1)
    {
        ...
        /*
         * 这里只置标志位，主循环里再处理 ADC1_ConvertedValue[]。
         */
    }
}
